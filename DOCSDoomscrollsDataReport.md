...
### 1.5 Field Loss Matrix (JSON -> Neon DB)

This compares per-source JSON fields (from `data/<source>/`) to what is currently stored in Neon. Items listed here exist in JSON but are missing or not populated in Neon.

| Source | JSON Location | Fields Present in JSON but Missing in Neon |
|---|---|---|
| Bible | `data/bible/chunks.json` | `source_metadata.reference` (not stored) |
| Bible Translations | `data/bibletranslations/chunks.json` | `source_metadata.reference` (not stored) |
| PoetryDB | `data/poetrydb/chunks.json` | `source_metadata.linecount`, `source_metadata.title` (not stored) |
| Wikiquote | `data/wikiquote/chunks.json` | `source_metadata.wikiquote_url` (not stored) |
| New Advent | `data/newadvent/chunks.json` | `source_metadata.chapterTitle`, `source_metadata.sourceId` (not stored) |
| Sacred Texts | `data/sacredtexts/chunks.json` | `source_metadata.tradition` (not stored) |
| Perseus | `data/perseus/chunks.json` | `source_metadata.card` (not stored) |
| CCEL | `data/ccel/chunks.json` | `source_metadata.era`, `source_metadata.tradition` (not stored) |
| Standard Ebooks | `data/standardebooks/chunks.json` | `source_metadata.position_percent` (not stored) |
| Wikiquote | `data/wikiquote/authors.json` | `wikiquote_url`, `quote_count`, `tier`, `discovery_method` (no columns in Neon) |
| Most sources | `data/*/authors.json` | `bio`, `wikipedia_url` exist in JSON but `authors.bio` is NULL for all rows and `wikipedia_url` has no column |

Additional Neon gaps based on live counts:
- `chunks.type` is NULL for 8,113,638 rows (mostly Gutenberg).
- `chunks.source_chunk_id` is NULL for 7,101,832 rows (IDs exist in JSON).
- `works.source_url`, `works.full_text_url`, and `works.source_list/source_sublist/source_bookshelf` are mostly or entirely NULL despite existing columns.
...
## 9) Resolution Plan Assessment (priority, difficulty, and data availability)

This section ties the resolution plan items to (a) importance, (b) difficulty, and (c) whether the needed data already exists in JSON source files.

### 9.1 Priority and Difficulty (summary)

| Item | Priority | Difficulty |
|---|---|---|
| Rights/licensing + takedown | Highest | Medium-High |
| Provenance/audit trail | High | High |
| Work/Edition model | High | High |
| Global dedup (exact + near-dup) | High | High |
| Editorial overrides + validation | Medium | Medium |

### 9.2 Does the JSON already contain the data?

| Item | Is data already in JSON? | Notes |
|---|---|---|
| Rights/licensing | Mostly no | Rights basis and territory flags are not in JSON; requires new metadata mapping |
| Provenance/audit trail | Partial | Some URLs exist in per-source configs and scripts, not in JSON; checksums/ingest_run are missing |
| Work/Edition model | Partial | Some edition-like info exists in Bible translations; most sources lack explicit edition metadata |
| Global dedup | No | Requires new hashes/clusters computed from text |
| Editorial overrides | No | Needs a new patch layer; JSON does not include corrections |

### 9.3 Does this answer any resolution plan questions?

Yes, this comparison clarifies:
- Rights/licensing and provenance are not solvable from existing JSON alone.
- Some provenance fields can be backfilled (source URLs, lists, phases) from scripts/configs without re-ingesting text.
- Chunk metadata loss is real (see Section 1.5), so any plan that needs rich context (offsets, verse references, chapter titles) will require either re-ingest or a JSON-to-Neon backfill step.

---

## 10) Rights + Provenance Implementation Package

This section implements the three requested items: (1) a per-source checklist, (2) a minimal schema, and (3) a no-reingest plan.

### 10.1 Rights/Provenance Checklist (per source)

| Source | Rights Basis to Capture | Provenance Fields to Capture |
|---|---|---|
| Gutenberg | PD basis by jurisdiction; translation status | download URL, ingest timestamp, checksum, parser version, ingest run ID |
| Standard Ebooks | PD basis + SE catalog notes | book page URL, EPUB URL, checksum, ingest run ID |
| CCEL | PD or permission basis | source URL(s), ingest timestamp, checksum |
| Perseus | translation rights + edition | TEI file path/URL, checksum, parser version |
| Sacred Texts | translation rights basis | page URL(s), ingest timestamp, checksum |
| New Advent | PD basis + site terms | chapter URLs, ingest timestamp, checksum |
| Bible (KJV) | PD basis | API URL per chapter, ingest timestamp |
| Bible API (WEB/ASV/YLT/DBY) | translation rights basis | API URL per chapter, ingest timestamp |
| PoetryDB | mixed rights; needs policy | API URL, ingest timestamp |
| Wikiquote | mixed rights; needs policy + attribution | page URL, ingest timestamp |

### 10.2 Minimal Schema (additive)

These are additive tables/columns that do not affect current app behavior.

tables:
- `sources` (id, name, homepage_url, license_url, default_rights_basis, notes)
- `ingest_runs` (id, source_id, started_at, finished_at, parser_version, notes)
- `artifacts` (id, source_id, work_id, download_url, downloaded_at, checksum, file_format, ingest_run_id)
- `rights` (id, rights_basis, license, territory, translator, translator_year, attribution_text, takedown_status, takedown_reason)

links:
- `works.rights_id` (nullable)
- `works.artifact_id` (nullable)
- `chunks.rights_id` (nullable, optional override)

### 10.3 No-Reingest Plan (safe backfill)

1) **Backfill from existing JSON/scripts:**
   - Populate `sources` and `ingest_runs` for each source.
   - Use known URLs in scripts/config to populate `artifacts.download_url` where possible.
2) **Best-effort checksums:**
   - For already-ingested items, compute checksums only if you can re-download without changing data.
3) **Rights tagging (policy-first):**
   - Record rights basis at the work level (PD, unknown, CC, etc.).
   - Add explicit takedown flags without removing data yet.
4) **No query changes:**
   - Keep app queries unchanged until you explicitly opt in.

---

## 11) MVP Low-Risk Plan (fast, additive)

These are the quickest, lowest-risk steps to address the top priorities without re-ingestion or app behavior changes. Each item can be accomplished via small scripts plus additive columns/tables.

1) **Rights/takedown (MVP):** add `works.rights_basis` + `works.takedown_status`; backfill defaults per source.
2) **Provenance URLs:** populate `works.source_url` and `works.full_text_url` via known URL patterns (no network lookup).
3) **Light canonicalization:** add `works.canonical_work_id` and fill only for obvious duplicates (same author/title across sources).
4) **Edition tags (MVP):** add `works.edition_label` + `works.edition_source` and populate from source/translation.
5) **Exact-dup marking:** compute a normalized text hash per chunk; store in a new column/table and surface in admin.
6) **Minimal validation:** add a QA script to flag empty/too-short/too-long chunks and unusual unicode.

Implementation note: these can be done as standalone scripts that write only to new columns/tables and do not alter existing text or IDs.

### 11.1 User-Facing Metadata (optional)

These fields can be surfaced in a passage detail panel without changing the core feed:
- Source label (Gutenberg, Standard Ebooks, Perseus, etc.).
- Edition label (e.g., ASV 1901, Perseus Eng2).
- Full text link (when available).
